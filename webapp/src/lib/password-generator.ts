...
export type EmailMode = 'plusAddressed' | 'catchAll' | 'subdomain' | 'forwarded';
export type ForwardedAliasType = 'word' | 'uuid' | 'custom';
/**
 * One config slot per provider type, mirroring Bitwarden's forwarded-email
 * forwarders. MailPal and SimpleLogin speak the same protocol and therefore
 * share the `simplelogin` slot; future providers (addy, duckduckgo, ...)
 * each get their own slot and can be configured side by side.
 */
export type ForwarderProvider = 'simplelogin';

export interface ForwarderAccountConfig {
  serverUrl: string;
  apiKey: string;
  aliasType: ForwardedAliasType;
  prefix: string;
  note: string;
}
...
export interface ForwardedEmailOptions {
  provider: ForwarderProvider;
  simplelogin: ForwarderAccountConfig;
}
...
export const defaultGeneratorSettings: GeneratorSettings = {
  ...
  forwarded: {
    ...
    simplelogin: { serverUrl: '', apiKey: '', aliasType: 'word', prefix: '', note: 'Generated by NodeWarden' },
  },
...
export function normalizeGeneratorSettings(value: unknown): GeneratorSettings {
  ...
  // Forwarder account settings, oldest shape first:
  // 1. flat fields on the legacy email block, 2. flat `forwarded` block,
  // 3. the current `{ provider, simplelogin }` shape.
  const forwardedRaw = stored.forwarded && typeof stored.forwarded === 'object'
    ? stored.forwarded as unknown as Record<string, unknown>
    : {};
  const accountSource: unknown = forwardedRaw.simplelogin && typeof forwardedRaw.simplelogin === 'object'
    ? forwardedRaw.simplelogin
    : forwardedRaw.serverUrl !== undefined || forwardedRaw.apiKey !== undefined
      ? forwardedRaw
      : normalizeLegacyForwardedFields(stored.email);
  const modes: GeneratorMode[] = ['password', 'passphrase', 'pin', 'username', 'email', 'sshKey'];
  const emailModes: EmailMode[] = ['plusAddressed', 'catchAll', 'subdomain', 'forwarded'];
  ...
  return {
    ...
    forwarded: {
      provider: 'simplelogin',
      simplelogin: normalizeAccountConfig(accountSource),
    },
    sshKey: {
...
function normalizeServerUrl(value: unknown): string {
  ...
  if (!raw) return '';
  // Relative base (e.g. "/mailpal") — resolved against the app origin, so a
  // reverse proxy on the same host works without hardcoding a hostname.
  if (raw.startsWith('/')) return raw.replace(/\/+$/, '');
  const withScheme = /^https?:\/\//i.test(raw) ? raw : `https://${raw}`;
  return withScheme.replace(/\/+$/, '');
}

function normalizeAccountConfig(value: unknown): ForwarderAccountConfig {
  const raw = value && typeof value === 'object' ? value as Partial<ForwarderAccountConfig> : {};
  return {
    serverUrl: normalizeServerUrl(raw.serverUrl),
    apiKey: String(raw.apiKey ?? '').slice(0, 512),
    aliasType: ['word', 'uuid', 'custom'].includes(raw.aliasType as ForwardedAliasType)
      ? raw.aliasType as ForwardedAliasType
      : defaultGeneratorSettings.forwarded.simplelogin.aliasType,
    prefix: String(raw.prefix ?? '').trim().slice(0, 64),
    note: String(raw.note ?? defaultGeneratorSettings.forwarded.simplelogin.note).trim().slice(0, 256),
  };
}

/**
 * Earlier builds stored forwarder-style fields on the email options object
 * (type 'forwarded' with serverUrl/apiKey siblings). Pull them forward so
 * those settings survive the move to the dedicated `forwarded` block.
 */
function normalizeLegacyForwardedFields(value: unknown): Partial<ForwarderAccountConfig> {
  if (!value || typeof value !== 'object') return {};
  const email = value as Partial<EmailOptions> & Partial<ForwarderAccountConfig>;
  if (email.type !== 'forwarded') return {};
  return { serverUrl: email.serverUrl, apiKey: email.apiKey, prefix: email.prefix };
}
...
export function generateForwardedPrefix(): string {
  return randomWord({ ...defaultGeneratorSettings.username, words: 1, capitalize: false, includeNumber: true });
}
...
export function generateValue(settings: GeneratorSettings): string {
  ...
  if (settings.mode === 'email') {
    // 'forwarded' aliases come from a remote provider and are generated
    // asynchronously by the page (see PasswordGeneratorPage).
    return settings.email.type === 'forwarded' ? '' : generateEmail(settings.email);
  }
